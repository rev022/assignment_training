...
/*	@RequestMapping(value = "/task2", method = RequestMethod.GET,produces = {"application/json","application/xml"})
	...
	@RequestMapping(value ="/notes/{notes}/isBookMarked/{isBookMarked}/taskid/{taskid}",method = RequestMethod.GET,produces = {"application/json","application/xml"})
	public List<Task> addNotesBM(@PathVariable String notes, @PathVariable String isBookMarked, @PathVariable int taskid, Model model) {
		model.addAttribute("notes", notes);
		model.addAttribute("isBookMarked", isBookMarked);
		model.addAttribute("taskid", taskid);
	int	row=task.addNotesBM(notes,isBookMarked,taskid);
		return task.printTask(taskid);
	}	
	}	
